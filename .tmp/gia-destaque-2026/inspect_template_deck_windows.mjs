...
import { importRuntimeModule } from "file:///C:/Users/jean.sanabia/.codex/plugins/cache/openai-primary-runtime/presentations/26.813.12317/skills/presentations/container_tools/runtime_helpers.mjs";
...
function zipNames(pptxPath) {
  return String(runCapture("tar", ["-tf", pptxPath], { encoding: "utf8" }))
    .split(/\r?\n/)
    .filter(Boolean);
}

function readZipText(pptxPath, entryName) {
  return Buffer.from(runCapture("tar", ["-xOf", pptxPath, entryName])).toString("utf8");
}

async function copyZipEntry(pptxPath, entryName, targetPath) {
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.writeFile(targetPath, Buffer.from(runCapture("tar", ["-xOf", pptxPath, entryName])));
}
